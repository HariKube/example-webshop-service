...
func init() {
	...
	utilruntime.Must(productv1.AddToScheme(scheme))
...
func main() {
	...
	options := ctrl.Options{
		...
		LeaderElectionID:       "462191c5.webshop.harikube.info",
	...
	if err := (&controller.PaymentReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&controller.LicenceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	// nolint:goconst
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1.SetupProductWebhookWithManager(mgr); err != nil {
	...
	// nolint:goconst
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1.SetupRegistrationRequestWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "RegistrationRequest")
			os.Exit(1)
		}
	}
	// nolint:goconst
